...
%plot particles with different color, only works with track_particles
plotstr = 'figure(fignum); scatter3(xscale*xyzLoc(L+M).time(:,1),zscale*(-z2+xyzLoc(L+M).time(:,3)),yscale*(ymax-xyzLoc(L+M).time(:,2)),30,''filled'',''CData'',xyzLoc(L+M).time(:,4));colormap(jet(125));';
tmp = z1;
z1 = max(tmp,z2);
z2 = min(tmp,z2);
