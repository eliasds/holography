...
for L = 2:length(xyzLocCentroidScaled)
    ...
    dist_mat = nan(m,n);
    multiWaitbar('B: Creating Distance Matrix Between Frames Adjacent Frames...',0);
    for M = 1:m
        for N = 1:n
            dist_mat(M,N) = sqrt((mat1(M,1)-mat2(N,1))^2 + ...
                (mat1(M,2)-mat2(N,2))^2 + (mat1(M,3)-mat2(N,3))^2);
        end
        multiWaitbar('B: Creating Distance Matrix Between Frames Adjacent Frames...',M/m);
    end
    ...
    multiWaitbar('C: Filling in data for blinking particles...',0);
...
%             if L-i > 0 && particles(j).match(L-i,1) == 1
        ...
        multiWaitbar('C: Filling in data for blinking particles...',j/length(particles));
    end
    ...
    multiWaitbar('D: Creating NEW Distance Matrix Between Adjacent Frames...',0);
    for j = 1:partLength
        for i = 1:n
        ...
        multiWaitbar('D: Creating NEW Distance Matrix Between Adjacent Frames...',j/partLength);
    end
            
    
    %Append matches in L+1 to corresponding particles or adds them as new particles
    partLength = length(particles);
    multiWaitbar('E: Match Particles from Previous Frame...',0);
    for i = 1:n
        isInMatch = 0;  
...
%Switch && below?
        ...
        multiWaitbar('E: Match Particles from Previous Frame...',i/n);
    end
...
multiWaitbar('F: Removing Neglegible Particles and Correcting Data for Blinking Particles...',0);
index = 1; 
for i = 1:partLength
    ...
    multiWaitbar('F: Removing Neglegible Particles and Correcting Data for Blinking Particles...',i/partLength);
end
...
multiWaitbar('G: Reformatting Particle Data...',0);
for L = 1:numofframes
    index = 1;
    ...
    multiWaitbar('G: Reformatting Particle Data...',L/numofframes);
end
