...
xyzLocInput2 = xyzLocSmoothed2; % change to struct with your data
...
scatter3(xscale*xyzLocInput(La).time(:,1),zscale*(-zmin+xyzLocInput(La).time(:,3)),yscale*(ymax-xyzLocInput(La).time(:,2)),30,'filled','CData',xyzLocInput(La).time(:,4)); colormap(jet(125)); %Multicolored Particles
    ...
    drawnow
subplot(1,2,2)    
scatter3(xscale*xyzLocInput2(La).time(:,1),zscale*(-zmin+xyzLocInput2(La).time(:,3)),yscale*(ymax-xyzLocInput2(La).time(:,2)),30,'filled','CData',xyzLocInput2(La).time(:,4)); colormap(jet(125)); %Multicolored Particles
    view([az,el])
    axis equal
    axis([0,ceil(4*xscale*xmax)/4,0,floor(2*zscale*zmax)/2,0,ceil(4*yscale*ymax)/4]); %Round axis value
    xlabel('(mm)')
